...
import { Card, CardImg, Col, Badge, Row, Popover, OverlayTrigger } from 'react-bootstrap';
...
const MenuItemCard = ({ item, onPurchaseMenuItem, userId, onToggle }) => {
    ...
    return (
        <>
            <Card className="menu-item-card menu-accordion-card my-2" key={`${item.id}`}>
                <CardImg className="rounded img-responsive menu-item-img" src={item.imageUrl} alt="menu item" />
                <hr className="menu-card-divider m-0" />
                <Card.Body className="menu-list-text menu-list-body p-0" id={item}>
                    <p className="menu-list-text mt-3">
                    ...
                    <Row className="m-0 food-type-labels d-flex justify-content-start p-0">
                        <Col sm={6} md={6} lg={6} xl={6} className="p-0">
                    ...
                    <Row className="m-1">
                        <button type="button" className="btn btn-small btn-primary mt-1" onClick={onIngredientsClicked}>
